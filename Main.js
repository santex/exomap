...
draggingArea=function(){
	return !buttons.over0 && !buttons.over1;
}
...
cycle=function(){
	...
	if(starsDepthColor==null){
		...
		for(i=0;starsArray[i]!=null;i++){
			star = starsArray[i];
			if(star.s>0){
        var coll = ColorOperators.interpolateColors(starsDepthColor, starsColor, Math.min(Math.max(star.s*0.5, 0), 1));;
				context.fillStyle = coll;
				context.beginPath();
       ...
       context.fill();
        ...
        context.fillText(star.name ,star.p.x+ star.s*2+15, star.p.y-5);
	
        } 
				
        //context.arc(star.p.x+10, star.p.y+10,star.name);
      

			}else{//}
      }
      
			//spotColors(coordinates[i], radius[i], finalZoom*interpolatedDistances[i], starsColor, starsDepthColor);
		}
	}
}
...
// spotColors=function(p, r, d, color0, color1){
	// p = transformation(p, d);
	// if(p.z<0) return;
	// context.fillStyle = ColorOperators.interpolateColors(color1, color0, Math.min(Math.max(p.z*0.5, 0), 1));
	// context.beginPath();
	// context.arc(p.x, p.y, r*p.z, 0, TwoPi);
	// context.fill();
// }
